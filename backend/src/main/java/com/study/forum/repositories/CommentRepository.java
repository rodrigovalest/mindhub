...
public interface CommentRepository extends JpaRepository<Comment, UUID> {
    ...
    @Query(value =
            "WITH RECURSIVE CommentHierarchy AS " +
                    "( " +
                    "  SELECT id, md_text, parent_comment_id, creation_timestamp, update_timestamp, post_id, user_id " +
                    "  FROM tb_comments " +
                    "  WHERE post_id = :postId AND parent_comment_id IS NULL " +
                    "  UNION ALL " +
                    "  SELECT c.id, c.md_text, c.parent_comment_id, c.creation_timestamp, c.update_timestamp, c.post_id, c.user_id" +
